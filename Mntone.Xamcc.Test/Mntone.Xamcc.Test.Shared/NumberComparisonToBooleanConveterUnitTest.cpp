...
namespace Mntone { namespace Xamcc { namespace Test {
	...
	TEST_CLASS( PLATFORM_PREFIX( NumberComparisonToBooleanConveterUnitTest ) )
	{
	public:
		TEST_METHOD( ConvertBackEqualToTestMethod )
		{
			auto converter = ref new Converters::NumberComparisonToBooleanConveter();
	...
	private:
		template<typename N>
		void TestZero( Converters::NumberComparisonToBooleanConveter^ converter, function<bool( N )> comparisonFunc )
		{
			...
			for( auto i = 0; i < 1000; ++i )
			{
				...
				Assert::AreEqual( comparisonFunc( n ), castedRet->Value );
			}
		}

		template<typename N>
		void TestNonZero( Converters::NumberComparisonToBooleanConveter^ converter, N x, function<bool( N, N )> comparisonFunc )
		{
			random_device seedGenerator;
			auto getNewNumber = bind( uniform_int_distribution<N>( numeric_limits<N>::min(), numeric_limits<N>::max() ), mt19937_64( seedGenerator() ) );
			...
			{
				auto n = getNewNumber();
				auto ret = converter->Convert( n, bool::typeid, x.ToString(), nullptr );
				auto castedRet = dynamic_cast<IBox<bool>^>( ret );
				Assert::IsNotNull( castedRet );
				Assert::AreEqual( comparisonFunc( n, x ), castedRet->Value );
			}
		}

		template<typename N>
		void TestZeroF( Converters::NumberComparisonToBooleanConveter^ converter, function<bool( N )> comparisonFunc )
		{
			random_device seedGenerator;
		...
		template<typename N>
		void TestNonZeroF( Converters::NumberComparisonToBooleanConveter^ converter, N x, function<bool( N, N )> comparisonFunc )
		{
			random_device seedGenerator;
			auto getNewNumber = bind( uniform_real_distribution<N>( numeric_limits<N>::min(), numeric_limits<N>::max() ), mt19937_64( seedGenerator() ) );
